...
- **自动解析**：消息中出现 Lofter 帖子链接时，自动提取内容。按 LOFTER 返回的帖子类型分流——图片贴返回图片消息链，文字贴显示字数并提取完整正文（长文整理成 .txt 文件），视频贴下载后作为文件发出
- **搜索**：按关键词搜索 Lofter 内容
...
**视频贴**：先发标题/作者/标签，再把视频下载到本地作为文件发送，文件名取文章标题。超过 `video_max_mb` 或下载失败时只发文字并说明原因。

...
- **自动解析**：消息中出现 Lofter 帖子链接时，自动提取内容。按 LOFTER 返回的帖子类型分流——图片贴返回图片消息链，文字贴显示字数并提取完整正文（长文整理成 .txt 文件），视频贴下载后作为视频消息发出
- **搜索**：按关键词搜索 Lofter 内容
...
| `text_file_threshold` | 文字贴超过这个字数时改发 .txt 文件 | 1000 |
| `video_max_mb` | 解析视频贴时允许下载的最大体积（MB） | 100 |
| `temp_file_ttl_hours` | 生成的 .txt 和视频文件保留多久（小时），最小 1 | 1 |
...
图片贴 / 文字贴 / 视频贴由 LOFTER 返回的 `post_type` 字段判定（1=文字、2=图片、4=视频），不再靠「有没有图」来猜。遇到未收录的类型（3/5/6）才退回按有无图片判断。
...
**视频贴**：先发标题/作者/标签，再把视频下载到本地作为视频消息发送，可以直接在聊天里播放。超过 `video_max_mb` 或下载失败时只发文字并说明原因。

**文字贴**：标题行会带上字数（优先用 LOFTER 返回的字数，缺失时按正文长度计）。

- 字数在 `text_file_threshold`（默认 1000）以内：以合并转发消息发送，第一条为标题/作者/字数/标签，中间为正文分段内容，最后一条为原链接
- 超过阈值：把全文整理成 .txt 文件发送，**文件名取文章标题**，文件开头附作者、标签、字数和原文链接。不再折叠分段，长文不会被截断

生成的 .txt 放在数据目录的 `articles/` 下，文件名是 `帖子ID_标题_解析时间.txt`——帖子 ID 打头保证不同文章不会撞名（《无题》《第一章》这类标题很常见），标题和时间方便直接翻目录。发到聊天里的文件名仍旧只取文章标题。
...
- **标签补抓**：持续翻页直到已知内容、空页，或翻到订阅之前发布的内容为止，保存抓取中断时的位置，下轮继续。依赖标签接口分页正常工作，重复页会记录错误
- **跨订阅去重**：通过 `sent_posts` 表记录会话维度已推送的帖子 ID，同一帖子匹配多个订阅标签时只推送一次
- **数据库访问串行化**：所有 SQLite 操作通过插件专用单线程执行器运行，避免多会话轮询和命令并发时共享连接被多线程同时访问
- **只推订阅之后的新内容**：推送前比对帖子发布时间与该订阅的创建时间，订阅之前发布的内容不推送，也不会把标签历史排进待发送队列。帖子同时命中多个订阅标签时按最早那次订阅算；缺少发布时间的帖子当作旧帖跳过
- **冷启动保护**：首次订阅及每次新增订阅时自动预热，不会刷屏推送历史帖子
- **临时文件定期清理**：后台任务每 10 分钟扫一遍 `articles/` 和 `videos/`，删掉超过 `temp_file_ttl_hours` 的成品和 `.part` 半成品；启动时先扫一次，上次退出留下的文件也能清掉。发送是异步的，删不掉正在占用的文件就跳过，下一轮再说

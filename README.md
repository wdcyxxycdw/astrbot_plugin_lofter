...
- **自动解析**：消息中出现 Lofter 帖子链接时，自动提取内容。按 LOFTER 返回的帖子类型分流——图片贴返回图片消息链，文字贴提取完整正文并以合并转发方式发送，视频贴下载后作为视频消息发出
- **搜索**：按关键词搜索 Lofter 内容
...
**文字贴**：以合并转发消息发送，第一条为标题/作者/标签，中间为正文分段内容，最后一条为原链接。正文超过 2 万字时最多发送前 10 块（约 2 万字），并附提示。
